...
func (s *BlogService) GetArticle_0(ctx *gin.Context) {
	...
	newCtx := metadata.NewIncomingContext(ctx, md)
	out, err := s.server.(BlogServiceHTTPServer).GetArticle(newCtx, &in)
	if err != nil {
		s.resp.Error(ctx, err)
		return
	}
...
func (s *BlogService) RegisterService() {
	...
	s.router.Handle("POST", "/article", s.CreateArticle_0)

	s.router.Handle("GET", "/article", s.GetArticle_0)
